...
export default class TestTopNavigationBar extends React.Component {
  ...
  private get additionalContent(): React.ReactNode {
    ...
    return (
      <>
        {showSearchBox && <Search value={searchValue} onChangeText={value => this.setState({searchValue: value})} placeholder="Search input text" style={showSwitch ? {marginBottom: 16} : undefined} light={true} />}
  ...
  render(): React.ReactNode {
    ...
    return (
      <View style={styles.parentView}>
        <View style={{backgroundColor: getColor('tagBackgroundMagenta'), height: 20}} />
        <TopNavigationBar {...this.navigationProps} />
        <ScrollView keyboardShouldPersistTaps="handled" contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.container} style={styles.view}>
          <Text style={{marginBottom: 16}} type="heading-04" text="Top navigation bar" />
          <Text style={{marginTop: 16}} type="label-02" text="Left side setup" />
          ...
          <Text style={{marginTop: 16}} type="label-02" text="Right side setup" />
