...
import { Checkbox, Text, Notification, NotificationTypes, Link } from 'carbon-react-native';
  ...
  render(): React.ReactNode {
    ...
    return (
      <ScrollView keyboardShouldPersistTaps="handled" contentInsetAdjustmentBehavior="automatic" contentContainerStyle={styles.container} style={styles.view}>
        <Text style={{marginBottom: 16}} type="heading-04" text="Notification" />
        ...
        {types.map(type => {
          return <Notification
            ...
            actionArea={showAction ? <Link style={{alignSelf: 'flex-start'}} onPress={this.actionCallback} text="Action" /> : undefined}
            title={longTitle ? 'Awesome notification with a crazy long title that is really weird just to be long' : 'Awesome notification'}
            subTitle={showSubtitle ? longSubTitle ? 'Useful subtitle information about this notification with even longer info that will be useful for testing extreme edge cases.' : 'Useful subtitle information about this notification' : undefined}
            onDismiss={showDismiss ? this.onDismiss : undefined}
          />;
        })}
      </ScrollView>
    );
